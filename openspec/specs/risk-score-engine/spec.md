...
Motor de score de risco determinístico: uma função pura de `(sinais, config)` que calcula o score como a soma booleana ponderada dos 3 sinais válidos (`reuso_imagem`, `imei_serie`, `velocity`) usando os pesos e limiares da versão ativa da `scoring_config`, renormaliza com piso mínimo de cobertura, classifica a faixa por limiar versionado, filtra atributos proibidos e devolve um resultado estruturado sem fabricar score quando não puder avaliar.

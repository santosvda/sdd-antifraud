# claim-console Specification

## Purpose

Cliente web de operação/QA servido pela própria API que envia sinistros reais (`POST /sinistros`)
e exibe o ciclo do caso em tela (comprovante → stepper → caso + auditoria), consumindo o
`case-read-api` por polling. É ferramenta de apoio (não capacidade antifraude): não coleta
sinais, não calcula score e não decide mérito. Encarna os guardrails na apresentação —
"aceito para análise humana", nunca veredito — com cenários reproduzíveis e acesso graduado por
ambiente (default seguro). Faz fronteira com o Painel do Analista (Feature 2.7), que é a
superfície de decisão humana.

# risk-classification Specification

## Purpose

Classificação de risco a partir do score: mapeia o score numérico em uma faixa (baixo/médio/alto) por limiares versionados, gera uma explicação textual determinística e não-acusatória, nomeia os sinais ativados, e trata fail-open e anomalias (score fora de faixa, config indisponível/corrompida) sem inventar classificação, carimbando um motivo tipado de sem-classificação.
